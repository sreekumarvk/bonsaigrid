...
To keep this tutorial focused on the core performance mechanics (Thread-Per-Core and Zero-Allocation), we skipped over several massive complexities that BonsaiGrid handles behind the scenes:
...
4. **Persistence & Crash Recovery:** In-memory grids are blazing fast but volatile. If the entire datacenter loses power, all data vanishes. BonsaiGrid implements a Durable Log (writing state changes sequentially to NVMe/SSD) so that if the whole cluster crashes, it can perfectly rebuild its memory state upon reboot.
5. **Linearizability & Distributed Locks (CP Subsystem):** Sometimes you need strict, unbending consistency (like preventing two users from buying the last concert ticket). BonsaiGrid uses an embedded Raft consensus subsystem to offer Strongly Consistent primitives like Distributed Locks and Atomic Counters without needing an external system like ZooKeeper or etcd.
6. **Server-Side Queries & Indexing:** If you need to find all users with `age > 30`, fetching all 500GB of data over the network to your client to filter it would take hours. Instead, BonsaiGrid pushes the query down into the server using an embedded execution engine and secondary indexes, filtering the data at the source without ever deserializing objects.
7. **Security, mTLS, & RBAC:** In enterprise environments, data traveling between machines must be encrypted (mTLS). Furthermore, Role-Based Access Control (RBAC) ensures that Client A can only read from a map, while Client B is authorized to write to it.
