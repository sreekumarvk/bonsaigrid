...
#[no_mangle]
pub extern "system" fn Java_com_bonsaigrid_BonsaiGrid_startServer(
    _env: JNIEnv,
    _class: JClass,
) {
    thread::spawn(move || {
        ...
        let cluster = Arc::new(Cluster::new(
            vec![MemberInfo::new((1, 1), "127.0.0.1".into(), BASE_PORT, MEMBER_BASE, 0)],
            0,
            1,
        ));
        ...
        let _ = server::reactor::run(
            ...
            move |msg, conn_id, out| {
                md.inc_request();
                server::handlers::dispatch_bytes(msg, conn_id, &store, &cfg, &broker, &schemas, &cluster, None, &executor, &txn_service, &jet_service, out)
            },
