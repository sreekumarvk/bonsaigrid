...
pub trait EntryProcessor {
    ...
    fn process(&self, key: &[u8], value: Option<&[u8]>) -> Result<(Option<Vec<u8>>, Option<Vec<u8>>), ProcessorError>;
}
...
pub fn parse_processor(_data: &[u8]) -> Result<Box<dyn EntryProcessor>, ProcessorError> {
    ...
    Err(ProcessorError("EntryProcessor execution is not fully supported for this processor type in BonsaiGrid".to_string()))
}
